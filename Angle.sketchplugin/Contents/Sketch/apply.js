    ...
    _createClass(Angle, [{
        ...
        value: function () {
            function guessRotationAndReversion() {
                ...
                var artboardSize = this.artboard.frame();
...
var _SymbolicAngle = __webpack_require__(4);
...
var _PixelDensity = __webpack_require__(5);
...
function getSelectionAndOptions_forAngleInstances(artboards, angles, alertImage) {

    var anglesCount = angles.length;

    var array = Array.from({ length: anglesCount }, function (x, i) {
        return i;
    });
    ...
    if (array.length > 1) {
        var targetLabels = array.map(function (a, i, as) {
            ...
            var label = createLabel(angles[i].description(), 12, rectangle);
    ...
    var artboardNames = artboards.map(function (a) {
        return a.name();
    });
    var artboardImages = artboards.map(function (a) {
        return smallImagesFromArtboard(a);
    });
    var artboardSelections = array.map(function (a, index, as) {
        return popUpButtonsforRectangleIndexer_withTitleIndexer_andImageIndexer_defaultSelected_onIndex(function (i) {
    ...
    var compressionRatioNames = Object.values(_CompressionRatio.CompressionRatio).map(function (a) {
        return a.selectionLabel;
    });
...
exports['default'] = function (context) {
  ...
  var artboardsNSArray = context.document.artboards();
  var artboards = Array.fromNSArray(artboardsNSArray).filter(function (a) {
  ...
  if (errors.length == 0) {
    context.document.showMessage(_Error.Error.unsupportedElement.message);
  } else {
...
function applyAngles(options) {
  ...
  if (artboards.length == 1) {
    ...
    var response = Shared.getSelectionAndOptions_forAngleInstances(artboards, angles, angleLogo);
...
var _SymbolicAngle2 = __webpack_require__(4);
